...
describe("pose gates", () => {
  ...
  it("yaw 23° blocks (true 3/4)", () => {
    const res = analyze(rotatedInput(23, 0, 0));
    expect(res.ok).toBe(false);
    expect(res.gates.blocking.some((g) => g.code === "yaw")).toBe(true);
  });

  it("yaw 13° degrades but scores", () => {
    const res = analyze(rotatedInput(13, 0, 0));
    expect(res.ok).toBe(true);
    expect(res.gates.warnings.some((g) => g.code === "yaw-mild")).toBe(true);
  });

  it("yaw 8° scores cleanly (laptop-natural)", () => {
    const res = analyze(rotatedInput(8, 0, 0));
    expect(res.ok).toBe(true);
    expect(res.gates.blocking).toEqual([]);
    expect(res.gates.warnings.some((g) => g.code === "yaw-mild")).toBe(false);
  });

  it("pitch 17° blocks (looking down wrecks verticals)", () => {
    const res = analyze(rotatedInput(0, 17, 0));
    expect(res.ok).toBe(false);
    expect(res.gates.blocking.some((g) => g.code === "pitch")).toBe(true);
  });

  it("pitch 11° degrades but scores (laptop webcam pose)", () => {
...
describe("coerceMatrix16", () => {
  it("reads a JSON-revived Float32Array object", () => {
    const arr = toColMajor16(rotXYZ(10, 5, 0));
    const revived = JSON.parse(JSON.stringify(new Float32Array(arr))) as unknown;
    expect(Array.isArray(revived)).toBe(false);
    const coerced = coerceMatrix16(revived);
    expect(coerced).toHaveLength(16);
    expect(coerced![0]).toBeCloseTo(arr[0]!, 5);
    expect(coerced![15]).toBeCloseTo(arr[15]!, 5);
  });
});

describe("reanalyze quality gates", () => {
  it("does not refuse a downscaled JPEG that would fail face-too-small", () => {
    const live = analyze(canonicalInput());
    ...
    expect(jpeg.overall).not.toBeNull();
    expect(jpeg.gates.blocking.some((g) => g.code === "face-too-small")).toBe(
      false,
    );
  });

  it("still refuses a tiny face on a fresh capture", () => {
    const res = analyze(canonicalInput({ imageWidth: 200, imageHeight: 250 }));
    expect(res.ok).toBe(false);
    expect(res.gates.blocking.some((g) => g.code === "face-too-small")).toBe(
      true,
    );
  });
});
