...
export function updateScan(id: string, patch: Partial<StoredScan>): StoredScan | undefined {
  ...
  localStorage.setItem(
    SCANS_KEY,
    JSON.stringify([next, ...scans.filter((s) => s.id !== id)]),
  );
  return next;
}
...
export function getScan(id: string): StoredScan | undefined {
  const matches = loadScans().filter((s) => s.id === id);
  if (matches.length === 0) return undefined;
  return (
    matches.find((s) => s.result.ok && s.result.overall !== null) ?? matches[0]
  );
}

export function saveScan(scan: StoredScan): void {
  const scans = [scan, ...loadScans().filter((s) => s.id !== scan.id)].slice(0, MAX_SCANS);
  try {
