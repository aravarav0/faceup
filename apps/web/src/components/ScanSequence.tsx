...
import { LANDMARKS, pslFromScan } from "@freeharmony/engine";
import { PSL_COLOR } from "@/components/FaceRating";
...
export function ScanSequence({
  ...
  const psl = pslFromScan(result.overall, result.overallPercentile);
  const rating = psl?.rating ?? 0;
  const shownScore = rating * ringEased;
  const ringFill = (shownScore / 10) * 100;
          ...
          >
            ...
            >
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="numeral text-4xl">
                  {shownScore.toFixed(1)}
                  <span className="text-lg text-ink-2">/10</span>
                </span>
                <span
                  className="label-caps"
                  style={
                    landed && psl
                      ? { color: PSL_COLOR[psl.title] }
                      : undefined
                  }
                >
                  {landed && psl ? psl.title : "rating"}
                </span>
              </div>
            </div>
          </div>
