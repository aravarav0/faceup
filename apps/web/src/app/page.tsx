...
import { AREA_LABELS, pslFromScan } from "@freeharmony/engine";
  ...
  return (
    <main className="mx-auto w-full max-w-xl px-5 py-6 flex flex-col gap-5">
      <header className="flex items-center justify-between">
        <h1 className="font-display text-2xl">FaceUp</h1>
      ...
      {last && last.result.overall !== null && (
        <Link href={`/results/${last.id}`} className="card px-5 py-4 flex items-center gap-4 hover:border-gold/40 transition-colors">
          <ScoreRing
            score={pslFromScan(last.result.overall, last.result.overallPercentile)?.rating ?? null}
            max={10}
            decimals={1}
            size={64}
          />
          <div className="flex-1">
            <p className="label-caps mb-1">Last Face Rating</p>
            {(() => {
              const psl = pslFromScan(last.result.overall, last.result.overallPercentile);
              return psl ? (
                <p className="text-sm mb-1" style={{ color: PSL_COLOR[psl.title] }}>
