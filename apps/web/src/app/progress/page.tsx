...
import { pslFromScan } from "@freeharmony/engine";
          ...
          {scans.map((s) => (
            <div key={s.id} className="card flex items-center gap-4 px-4 py-3">
              ...
              <div className="flex-1">
                {(() => {
                  const psl = pslFromScan(s.result.overall, s.result.overallPercentile);
                  return (
                    <>
                      <p className="numeral text-xl">
                        ...
                        ) : s.result.overall !== null ? (
                          `${s.result.overall.toFixed(1)}%`
                        ) : (
                          "—"
                        )}
                      </p>
                      <p className="text-xs text-ink-3">
                        {psl ? (
                          <span style={{ color: PSL_COLOR[psl.title] }}>{psl.title}</span>
                        ) : null}
                        {psl ? " · " : ""}
